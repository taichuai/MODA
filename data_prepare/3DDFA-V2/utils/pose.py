...
def viz_pose(img, param_lst, ver_lst, show_flag=False, wfp=None, show_info=True):
    poses = []
    for param, ver in zip(param_lst, ver_lst):
        P, pose = calc_pose(param)
        img = plot_pose_box(img, P, ver)
        # print(P[:, :3])
        if show_info: print(f'yaw: {pose[0]:.1f}, pitch: {pose[1]:.1f}, roll: {pose[2]:.1f}')
        poses.append(pose)
...
def viz_pose2(img, param_lst, ver_lst, show_flag=False, wfp=None, show_info=True):
    poses = []
    cams = []
    scales = []
    for param, ver in zip(param_lst, ver_lst):
        P, pose, s = calc_pose2(param)
        img = plot_pose_box(img, P, ver)
        # print(P[:, :3])
        if show_info: print(f'yaw: {pose[0]:.1f}, pitch: {pose[1]:.1f}, roll: {pose[2]:.1f}')
        poses.append(pose)

...
def main(args):
    ...
    pose_wfp = f'examples/results/videos/{fn.replace(suffix, "")}_{args.opt}_smooth.npy'
    ...
    for i, frame in tqdm(enumerate(reader)):
        ...
        if len(queue_ver) >= n:
            ...
            if args.opt == '2d_sparse':
                ...
                # print('----')
                # print(ver_ave)
                img_draw, poses = viz_pose(queue_frame[n_pre], param_lst, [ver_ave], show_flag=False, show_info=False)
                poses_lst.append(poses)
            else:
    ...
    if args.opt == 'pose':
        np.save(pose_wfp, np.array(poses_lst))
        print(f'Poses are saved to {pose_wfp}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-o', '--opt', type=str, default='2d_sparse', choices=['2d_sparse', '2d_dense', '3d', 'pose'])
